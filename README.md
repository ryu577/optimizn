...
#### Continuous Training
Both the `SimAnnealProblem` and the `BnBProblem` extend a superclass called `OptProblem` that has a `persist` function, which saves optimization problems to three folders: `DailyObj` (the parameters of the latest daily problem instance), `DailyOpt` (the latest daily problem instance and its most optimal solution), and `GlobalOpt` (problem instance with the most optimal solution, observed over all time). The user must specify the problem parameters in their subclass through a `params` attribute in order for their problem parameters, problem instances, and optimal solutions to be saved by continuous training.
